...
class RaffleBot:
    ...
    def list_active_giveaways_from_html(self, log=None):
        """
        Folose?te Playwright headless browser pentru a scrape rafle ?i a intra în ele.
        ...
        try:
            ...
            with sync_playwright() as p:
                ...
                # Încarca cookies din Redis (Upstash)
                cookies = self.load_cookies(log)
                if cookies:
                ...
                # A?teapta ca raflele sa se încarce (Vue.js e lent)
                ...
                # Converte?te în format compatibil
                result_giveaways = []
                for g in giveaways:
    ...
    def join_giveaway(self, ref, log=None):
        """
        Intra în rafla folosind Playwright (API-ul e blocat).
        ...
        try:
            ...
            with sync_playwright() as p:
                ...
                # A?tepta ca Vue.js sa se încarce
                page.wait_for_timeout(5000)
                
                # Verifica daca deja înscris
        ...
        try:
            ...
            for g in giveaways:
                try:
                    ...
                    # Folose?te DOAR Playwright pentru a verifica ?i intra în rafla
