...
def start_gui(bot_logic):
    def main(page: ft.Page):
        # â”€â”€ SetÄƒri generale â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        ...
        # â”€â”€ Elemente UI â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        ...
        # â”€â”€ FuncÈ›ii de log È™i update â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        def log(msg):
        ...
        def on_qr_bytes(data):
            ...
            if data:
                try:
                    ...
                    qr_status.value = "ScaneazÄƒ codul QR cu Steam Mobile"
                    qr_status.color = "#3FB950"
                except Exception as e:
                    qr_status.value = f"Eroare: {e}"
                    qr_status.color = "#F85149"
            else:
                qr_status.value = "QR indisponibil - Ã®ncearcÄƒ din nou"
                qr_status.color = "#D29922"
            page.update()

        def start_qr(e):
            qr_status.value = "Se genereazÄƒ QR..."
            qr_status.color = "#58A6FF"
            page.update()
            threading.Thread(target=bot_logic.get_steam_qr, args=(on_qr_bytes,), daemon=True).start()

        btn_qr = ft.ElevatedButton("GenereazÄƒ QR Steam", on_click=start_qr,
                                   style=ft.ButtonStyle(bgcolor="#3B82F6", color="white"))

        # â”€â”€ Butoane Steam â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        async def open_steam(e):
            await page.launch_url("https://store.steampowered.com/login/")
            update_status("âœ… Tab Steam deschis", "#3FB950")

        async def open_tms(e):
            await page.launch_url("https://takemyskins.com/")
            update_status("âœ… Tab TakeMySkins deschis", "#3FB950")
        ...
        # â”€â”€ Verificare manualÄƒ â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        def run_check(e):
            progress.visible = True
            runtime_dot.bgcolor = "#58A6FF"
            runtime_text.value = "RuleazÄƒ..."
            page.update()

            def worker():
                try:
                    result = bot_logic.run_check(is_headless=True, log_func=log)
                    update_status(f"âœ… Verificare terminatÄƒ: {result}", "#3FB950")
                except Exception as ex:
                    update_status(f"âŒ Eroare: {ex}", "#F85149")
        ...
        btn_check = ft.ElevatedButton("RuleazÄƒ verificarea", on_click=run_check,
                                      style=ft.ButtonStyle(bgcolor="#238636", color="white"))

        # â”€â”€ Statistici (safe) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        def build_stats():
            try:
                total, wins = bot_logic.db.get_stats()
            except Exception as e:
                return ft.Text(f"âš ï¸ Nu s-au putut Ã®ncÄƒrca statisticile: {e}",
            ...
            if wins:
                ...
                return ft.Column([
                    cards,
                    ft.Text("Niciun cÃ¢È™tig Ã®ncÄƒ. Ele vor apÄƒrea automat aici.",
        ...
        # â”€â”€ Card generic â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        ...
        refresh_btn = ft.ElevatedButton(
            ...
            style=ft.ButtonStyle(color="#8B949E", bgcolor="#161B22"),
        )
            ...
            ft.Container(
                ...
                content=ft.Column([
                    ...
                    ft.Row([
                        card(
                            "ðŸ” Login Steam",
                            "ConecteazÄƒ-te prin QR sau deschide manual",
                            ft.Column([
                        ...
                        card(
                            "ðŸš€ Verificare",
                            "RuleazÄƒ manual sau aÈ™teaptÄƒ programarea",
                            ft.Column([
                                btn_check,
                                status_text,
                    ...
                    card(
                        "ðŸ“Š Statistici",
                        "Rezultatele È™i istoricul",
                        stats_area,
                        right=refresh_btn,  # <--- butonul corectat
                    ),
                    card(
                        "ðŸ“‹ Log",
                        "IeÈ™irea Ã®n timp real",
                        log_box,
                    ),
                    ft.Text("v1.0 Â· Render Free", size=10, color="#21262D", text_align=ft.TextAlign.CENTER),
